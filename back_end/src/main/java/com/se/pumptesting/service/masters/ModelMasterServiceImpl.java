...
import com.se.pumptesting.dtos.ModelMasterDto;
...
@Service
@Transactional
public class ModelMasterServiceImpl implements MasterService {

	static Logger log = Logger.getLogger(ModelMasterServiceImpl.class.getName());
	...
	public ApplicationResponse addRecord(String requestPayload) {
		...
		try {
			Tbl00ModelMaster model = new Tbl00ModelMaster();
			ModelMasterDto dto = new ModelMasterDto();
			ObjectMapper mapper = new ObjectMapper();
			dto = mapper.readValue(requestPayload, ModelMasterDto.class);
	...
	public ApplicationResponse updateRecord(String requestPayload) {
		...
		try {
			...
			ObjectMapper mapper = new ObjectMapper();
			dto = mapper.readValue(requestPayload, ModelMasterDto.class);
			Tbl00ModelMaster model = (Tbl00ModelMaster) genericDao.getRecordById(Tbl00ModelMaster.class, dto.getId());
			BeanUtils.copyProperties(dto, model);
			model.setClientId(velotechUtil.getClientId());
	...
	public ApplicationResponse deleteRecords(List<Integer> ids) {
		...
		ApplicationResponse applicationResponse = null;
		try {
			genericDao.deleteAll(Tbl00ModelMaster.class, ids);
	...
	public ApplicationResponse getRecordsWithPagination(RequestWrapper requestWrapper) {
		...
		try {
			...
			applicationResponse = genericDao.getRecordsWithPagination(Tbl00ModelMaster.class, requestWrapper.getExtPagination(),
					GenericUtil.getConjuction(Tbl00ModelMaster.class, requestWrapper.getSearchCriterias()));
			List<Tbl00ModelMaster> models = (List<Tbl00ModelMaster>) applicationResponse.getData();
			long total = applicationResponse.getTotal();

			List<ModelMasterDto> dtos = getData(models);
	...
	private List<ModelMasterDto> getData(List<Tbl00ModelMaster> models) {

		List<ModelMasterDto> dtos = new ArrayList<>();
		try {
			for (Tbl00ModelMaster model : models) {
				ModelMasterDto dto = new ModelMasterDto();
